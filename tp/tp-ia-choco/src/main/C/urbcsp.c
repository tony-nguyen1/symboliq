...
int main(int argc, char* argv[])
{
  ...
  if (argc != 6)
    {
      printf("\n");
      printf("\tusage: urbcsp nbVariables tailleDomaine nbConstraints nbTuples nbInstances\n");
      ...
      printf("\t       l'ajout de \'\'> instances.txt\'\' vous permet de stocker\n");
      printf("\t              le réseau généré dans un fichier \'\'instances.txt\'\'\n");
      printf("\n");
      return 0;
    }
...
int MakeURBCSP(int N, int D, int C, int T, long *Seed)
{
  ...
  for (c=0; c<C; ++c)
    ...
    //  if(c==0) fprintf(stderr,"%d tuples de valeurs générés\n",i);
